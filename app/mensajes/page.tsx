...
export default function MessagesPage() {
  ...
  async function send() {
    ...
    const { error: insertError } = await supabase.from('messages').insert({ conversation_id: selected, sender_id: user.id, body })
    if (insertError) setError(insertError.message)
    else { setText(''); await loadMessages() }
    setSending(false)
  }
  ...
  return <main className="mx-auto max-w-6xl px-4 py-10">
    ...
    <div className="mt-8 grid min-h-[560px] overflow-hidden rounded-2xl border bg-card md:grid-cols-[320px_1fr]">
